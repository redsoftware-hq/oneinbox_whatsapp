    ...
    >
      ...
      >
        ...
        <div class="flex gap-2 justify-between">
          ...
          <div v-else-if="whatsapp.content_type == 'document'"
          ...
          <div v-else-if="whatsapp.content_type == 'video'"
...
function reactOnMessage(name, emoji) {
  createResource({
    url: 'frappe_whatsapp.api.whatsapp.react_on_whatsapp_message',
    params: {
      emoji,
      reply_to_name: name,
    },
...
const groupedMessages = computed(() => {
  return props.messages.reduce((acc, message) => {
    const date = formatDate(message.creation);
    if (!acc[date]) {
      acc[date] = [];
    }
...
const formatDate = (dateString) => {
  ...
  if (isToday(lastInteraction)) {
    ...
    return format(lastInteraction, 'dd MMM');
  } else {
    return format(lastInteraction, 'dd/MM/yy');
  }
};
